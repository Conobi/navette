...
struct H3CoroServer(Movable):
    ...
    def _on_request(mut self, var ev: H3Event) raises:
        ...
        # Drain user headers directly into req_headers; host appended after.
        while len(fields) > 0:
    ...
    def _drain_responses(mut self, now: UInt64) raises:
        ...
        for i in range(len(stream_ids)):
            ...
            if not ctx.headers_sent and not ctx.resp_writer._has_status():
                ctx_ptr.init_pointee_move(ctx^)
                continue
            # Send response headers — bypass List[QpackHeaderField] via
            # H3Connection.send_response_headers (saves ~2 String copies per header).
