...
struct Cubic(ImplicitlyCopyable, Movable):
    ...
    def on_packet_acked(
        ...
        if target > self.cwnd and self.cwnd > UInt64(0):
            var incr = (self.max_datagram_size * (target - self.cwnd)) // self.cwnd
            self.cwnd += incr
            if self.cwnd > CUBIC_MAX_CWND:
                self.cwnd = CUBIC_MAX_CWND
    ...
    def on_packets_lost(
        ...
        if self.cwnd < self.w_last_max:
            ...
            self.w_max = (
                self.cwnd * (CUBIC_BETA_NUM + CUBIC_BETA_DEN)
        ...
        var reduced = (self.cwnd * CUBIC_BETA_NUM) // CUBIC_BETA_DEN
