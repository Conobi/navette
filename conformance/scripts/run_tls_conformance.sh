#!/usr/bin/env bash
# Runs the tls-conformance-scenarios crate against examples/hello_h3_server and
# gates on pass_count >= conformance/tls_conformance_min_pass.txt.
#
# Exit codes:
#   0 — passes >= threshold
...
SCENARIOS_DIR="$CONFORMANCE_DIR/tls-conformance"
THRESHOLD_FILE="$CONFORMANCE_DIR/tls_conformance_min_pass.txt"
SERVER_DIR="$REPO_ROOT/examples/hello_h3_server"
SERVER_BIN="$SERVER_DIR/hello_h3_server"

# Port configurability: child scenario binaries read TLS_SERVER_PORT to locate
# the server (via tls_conformance_scenarios::server_addr()).
PORT="${TLS_SERVER_PORT:-4433}"
export TLS_SERVER_PORT="$PORT"

# Preflight — exit 2 on misconfig.
[[ -d "$SCENARIOS_DIR" ]] || { echo "[run_tls] missing $SCENARIOS_DIR" >&2; exit 2; }
[[ -f "$THRESHOLD_FILE" ]] || { echo "[run_tls] missing $THRESHOLD_FILE" >&2; exit 2; }
command -v cargo >/dev/null 2>&1 || { echo "[run_tls] cargo not in PATH" >&2; exit 2; }
...
if ss -ulnp 2>/dev/null | grep -q ":$PORT "; then
    echo "[run_tls] port $PORT already bound — refusing to spawn server" >&2
    exit 2
fi
...
ensure_server_fresh() {
    if [[ ! -x "$SERVER_BIN" ]]; then
        echo "[run_tls] $SERVER_BIN not built; rebuilding" >&2
    else
        local newest_src
        ...
        echo "[run_tls] navette source newer than $SERVER_BIN; rebuilding" >&2
    fi
    ...
    (
        ...
        [[ -n "$whl" ]] || { echo "[run_tls] could not locate built wheel" >&2; exit 2; }
        unzip -p "$whl" '*/hello_h3_server' > hello_h3_server || exit 2
        chmod +x hello_h3_server
    )
}

if [[ "${RUN_TLS_SKIP_REBUILD:-0}" == "1" ]]; then
    [[ -x "$SERVER_BIN" ]] || { echo "[run_tls] $SERVER_BIN missing and RUN_TLS_SKIP_REBUILD=1" >&2; exit 2; }
else
    ensure_server_fresh || { echo "[run_tls] failed to rebuild $SERVER_BIN" >&2; exit 2; }
fi

THRESHOLD="$(tr -d '[:space:]' < "$THRESHOLD_FILE")"
[[ "$THRESHOLD" =~ ^[0-9]+$ ]] || { echo "[run_tls] threshold is not an integer: '$THRESHOLD'" >&2; exit 2; }

# Create the log directory up front so we can capture build diagnostics too.
LOGDIR="${RUN_TLS_LOGDIR:-/tmp/run_tls.$$}"
mkdir -p "$LOGDIR"
...
echo "[run_tls] building scenarios (release)..."
(cd "$SCENARIOS_DIR" && cargo build --release --locked --bins) > "$LOGDIR/cargo.log" 2>&1 || {
    echo "[run_tls] cargo build failed — see $LOGDIR/cargo.log" >&2
...
if [[ -z "$MODULAR_LIB" ]]; then
    echo "[run_tls] could not locate modular/lib under $SERVER_DIR/.venv — run 'uv sync' in $SERVER_DIR" >&2
    exit 2
fi
...
BIND_TIMEOUT_S="${RUN_TLS_BIND_TIMEOUT_S:-15}"
...
if ! is_server_bound; then
    echo "[run_tls] server (pid $SERVER_PID) failed to bind $PORT within ${BIND_TIMEOUT_S}s" >&2
    cat "$SERVER_ERR" >&2
    exit 2
fi
...
SCENARIO_NAMES=$(cargo metadata --manifest-path "$SCENARIOS_DIR/Cargo.toml" --no-deps --format-version 1 \
    | python3 -c 'import json,sys; m=json.load(sys.stdin); pkg=next(p for p in m["packages"] if p["name"]=="tls-conformance-scenarios"); print("\n".join(t["name"] for t in pkg["targets"] if "bin" in t["kind"]))')

while IFS= read -r name; do
    [[ -n "$name" ]] || continue
    bin="$TARGET_DIR/$name"
    if [[ ! -x "$bin" ]]; then
        echo "[run_tls] missing built binary for scenario '$name' at $bin" >&2
        exit 2
    fi
    TOTAL=$((TOTAL + 1))
    if "$bin" > "$LOGDIR/scenario.$name.log" 2>&1; then
        PASSED=$((PASSED + 1))
        echo "[run_tls] PASS $name"
    else
        FAILED=$((FAILED + 1))
        echo "[run_tls] FAIL $name (see $LOGDIR/scenario.$name.log)"
    fi
done <<< "$SCENARIO_NAMES"

echo "[run_tls] passed=$PASSED failed=$FAILED total=$TOTAL threshold=$THRESHOLD"
echo "[run_tls] logs: $LOGDIR"

if (( PASSED < THRESHOLD )); then
    echo "[run_tls] REGRESSION: pass count $PASSED < threshold $THRESHOLD" >&2
    exit 1
fi
