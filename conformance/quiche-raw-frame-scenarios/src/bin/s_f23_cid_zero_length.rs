//! s_f23_cid_zero_length — F23.
//!
//! Sends a `NEW_CONNECTION_ID` frame whose `conn_id` is zero bytes long.
//! Per RFC 9000 §19.15, the issued connection ID length MUST be at least
//! one octet; a zero-length CID violates the encoding. Receivers MUST
//! close with `FRAME_ENCODING_ERROR`.
//!
//! Expected: transport-CC, code = `FRAME_ENCODING_ERROR (0x07)`, reason
//! contains `[QUIC-CID-ZERO-LENGTH]`.
...
const FAILURE_ID: &str = "F23";
const GUARD_TAG: &str = "[QUIC-CID-ZERO-LENGTH]";
