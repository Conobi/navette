//! s_f20_max_streams_overflow — F20.
//!
//! Sends a `MAX_STREAMS` frame with a value strictly greater than 2^60.
//! Per RFC 9000 §19.11, the `max` field of MAX_STREAMS MUST NOT exceed
//! 2^60 — a higher value would allow a stream id that does not fit in
//! the variable-length encoding. Receivers MUST close with
//! `FRAME_ENCODING_ERROR`.
//!
//! Expected: transport-CC, code = `FRAME_ENCODING_ERROR (0x07)`, reason
//! contains `[QUIC-MAX-STREAMS-OVERFLOW]`.
...
const FAILURE_ID: &str = "F20";
const GUARD_TAG: &str = "[QUIC-MAX-STREAMS-OVERFLOW]";
