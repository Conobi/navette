# conformance/lib/http2/hpack.mojo
#
...
struct HpackEncoder(Movable):
    ...
    Maintains a dynamic table and encodes lists of Header values
    into compressed HPACK wire bytes. Thread-unsafe: one encoder per
    connection.
    """
    ...
    def encode(mut self, headers: List[Header]) -> List[UInt8]:
        ...
        for i in range(len(headers)):
            ...
            # 1. Search static table for exact match -> indexed
            ...
            # 2. Search dynamic table for exact match -> indexed
    ...
    def _emit_literal_indexed(
        ...
        # If name_idx == 0, emit name as literal string
        if name_idx == 0:
            self._emit_string(wire, name)

        # Emit value as literal string
...
struct HpackDecoder(Movable):
    ...
    Maintains a dynamic table and decodes compressed header blocks
    into lists of Header values. Thread-unsafe: one decoder per
    connection.
    """
    ...
    def decode(
        ...
        Includes HPACK bomb protection via cumulative size check.
        """
        ...
        while pos < len(wire):
            ...
            if byte & 0x80:
                # Indexed Header Field (RFC 7541 Section 6.1): 1XXXXXXX
                ...
                if len(result[2]) > 0:
                    return (List[Header](), result[2])
                # RFC 7541 Section 6.3: new size MUST NOT exceed
                # the limit set by the protocol (SETTINGS).
    ...
    def _lookup_index(
        ...
        Indices 1-61 are in the static table.
        Indices >= 62 are in the dynamic table (62 = newest entry).
        """
        if index <= 61:
    ...
    def _decode_literal(
        ...
        The name may be indexed or literal; the value is always literal.
        """
        var consumed = 0
        ...
        # Decode value -- always literal string
    ...
    def _decode_string(
        ...
        Returns (string_value, bytes_consumed, error).
        """
        if pos >= len(wire):
