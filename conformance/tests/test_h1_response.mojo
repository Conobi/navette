...
def run_vector(v: PythonObject) raises -> Bool:
    ...
    if has_mode_flag:
        ...
        var mf_default_result = parse_response(wire, request_method, mf_default_config)
        ...
        if mf_default_behavior == "accept":
            check_accept_response(mf_default_result, mf_default_expected, vec_id + " [default]")
        else:
            check_reject_response(mf_default_result, mf_default_expected, vec_id + " [default]")
        ...
        var mf_flagged_result = parse_response(mf_wire2, request_method, mf_flagged_config)
        ...
        if mf_flagged_behavior == "accept":
            check_accept_response(mf_flagged_result, mf_flagged_expected, vec_id + " [flagged:" + flags_label + "]")
        else:
            check_reject_response(mf_flagged_result, mf_flagged_expected, vec_id + " [flagged:" + flags_label + "]")
