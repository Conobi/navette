//! F33 — RFC 9114 §7.2.1 (cluster C9).
//! DATA frame on a control stream MUST be H3_FRAME_UNEXPECTED.
//!
//! GUARD-TAG: [H3-DATA-ON-CTRL]
...
const EXPECTED_REASON_SUBSTRING: &str = "[H3-DATA-ON-CTRL]";
...
fn main() {
    ...
    let summary = match sync_client::connect(config, actions, None) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("s_f33: connect failed: {e:?}");
            std::process::exit(1);
        }
    };
    ...
    match summary.conn_close_details.peer_error() {
        Some(err) if err.is_app && err.error_code == H3_FRAME_UNEXPECTED => {
            ...
            if reason.contains(EXPECTED_REASON_SUBSTRING) {
                println!(
                    "s_f33: PASS code=0x{:x} reason={:?}",
                    err.error_code, reason
                );
                std::process::exit(0);
            }
            eprintln!(
                "s_f33: FAIL code OK (0x{:x}) but reason {:?} missing {:?}",
        ...
        Some(err) => {
            eprintln!(
                "s_f33: FAIL got code=0x{:x} is_app={}, expected H3_FRAME_UNEXPECTED (0x0103) app",
                err.error_code, err.is_app
            );
            std::process::exit(1);
        }
        None => {
            eprintln!("s_f33: FAIL no peer close");
            std::process::exit(1);
        }
    }
}
