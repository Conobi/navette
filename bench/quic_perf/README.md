...
TQUIC-style HTTP/3 benchmark comparing navette's `h3_server` to the TQUIC reference, under matched payloads, matched concurrency, and CPU-saturating load.

> **Status: calibrated.** End-to-end run validated against the TQUIC reference: `tquic_server` saturates core 0 at 87K req/s with `tquic_client` (88% CPU), confirming the harness exposes real saturation. `results/REFERENCE.md` has the current numbers; navette measures ~210× slower long-conn and ~2,500× slower short-conn than TQUIC on the same hardware, while using <6% of one core — the bottleneck is connection-establishment throughput, not steady-state stream work.

## What this measures

The throughput (req/s, MB/s) and server CPU% of navette's H3 stack vs `tquic_server` (pinned to upstream `v1.0.0`, commit `4dcec0f`) over loopback, using both `tquic_client` (multi-threaded, drives to saturation) and `h2load-h3` (single-threaded, kept for regression tracking against our prior numbers). Methodology mirrors https://tquic.net/docs/further_readings/benchmark/.
...
|---|---|---|
| Linux kernel ≥ 5.19 | `IORING_REGISTER_PBUF_RING` for navette's multishot recvmsg | `uname -r` |
...
# Single cell:
./scripts/bench.sh navette 5k long-conn tquic_client
...
- `scripts/start-server.sh <navette|tquic>` — starts the server in Docker pinned to core 0, mounts payloads + certs read-only, then calls `wait-ready.sh`.
...
- **Server CPU% < 80 with `h2load`**: expected. h2load is single-threaded and saturates client-side first. h2load numbers should be read as "did navette regress between commits," not "is navette within X% of TQUIC."
...
- **navette's `2m` rows look weak vs TQUIC**: expected until M4 flow-control auto-tuning lands. Mojo-net's M3 default stream FC window is 1 MiB, so 2 MiB transfers stall on `MAX_STREAM_DATA` updates.
...
- **2 MB payload is FC-bound on navette** until M4 stream-FC auto-tuning ships.
...
2. **Rebuild** the bench server:
   ...
   docker build -t navette-bench:profile -f bench/Dockerfile .
   ```

3. **Run** any bench cell as usual, e.g.:

   ```bash
   ./scripts/bench.sh navette 1k long-conn tquic_client --iters 3
   ```
