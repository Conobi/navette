...
**Implication for follow-on optimization spec:** the next pass should target `src/h3/qpack/decoder.mojo` directly. Candidate angles: static-table lookup is likely linear-scan (Mojo `Dict[String, Int]` over a 99-entry static table at 14k rps could explain the 50× cost); per-call allocation of the result `List[QpackHeaderField]`; varint length prefix decoding. The `_drain_stream` byte-shift / Dict-copy patterns visible in `_parse_frames_from_buf` (Topic 2's predicted optimization targets) account for only ~1% — they are below the bench harness sensitivity floor and are NOT the right target for a long-conn RPS lift.
