...
- `bench/build.sh` fails from worktrees because it infers `BOUCLE_DIR=$REPO_ROOT/../boucle` which doesn't exist relative to `.worktrees/baseline-main/`. Fix: pass explicit `BOUCLE_DIR=/home/donokami/Projets/perso/boucle SIMDJSON_DIR=/home/donokami/Projets/perso/json-simd-mojo` env vars (matches `feedback_bench_offbuild_image_hygiene.md` lesson).
