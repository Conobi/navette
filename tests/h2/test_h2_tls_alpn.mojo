...
def test_alpn_h2_negotiated() raises:
    ...
    cli_cfg.set_alpn_protocols(alpn_list)
    srv_cfg.set_alpn_protocols(alpn_list)
...
def test_alpn_fallback_h1() raises:
    ...
    cli_cfg.set_alpn_protocols(client_protos)

    var server_protos = List[String]()
    server_protos.append("http/1.1")
    srv_cfg.set_alpn_protocols(server_protos)
